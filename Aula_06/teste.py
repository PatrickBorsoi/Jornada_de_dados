    ...
    try:
        ...
        while lista_teste == False:
            ...
            for obra in df_tabela_mae['Id Obra']:
                ...
                try:
                    IFRAME_ARVORE_ARQUIVO = driver.find_element(By.CSS_SELECTOR, '#divIframeArvore > iframe')
                    driver.switch_to.frame(IFRAME_ARVORE_ARQUIVO)

                    LISTA_DOCUMENTOS = driver.find_elements(By.CLASS_NAME, 'infraArvoreNo')
                    if len(LISTA_DOCUMENTOS) != 0:
                        lista_invertida = reversed(LISTA_DOCUMENTOS)
                        nom_ultimo_arquivo = LISTA_DOCUMENTOS[-1].text
                    else:
                        LISTA_DOCUMENTOS = driver.find_element(By.CLASS_NAME, 'infraArvoreNo')
                ...
                for documento in lista_invertida:
                    try:
                        driver.switch_to.default_content()
                        IFRAME_CONTEUDO_DOCUMENTO = driver.find_element(By.CSS_SELECTOR, '#divIframeVisualizacao > iframe')
                        driver.switch_to.frame(IFRAME_CONTEUDO_DOCUMENTO)
                        IFRAME_CONTEUDO_BRANCO = driver.find_element(By.CSS_SELECTOR, '#divArvoreConteudo > #divArvoreHtml > #divArvoreInformacao')   
                    ...
                    try:
                        ...
                        processo_not_click_nao_possui = processo_not_click_texto.find('Processo não possui')
                        processo_not_click_nao_aberto = processo_not_click_texto.find('Processo aberto')
                        processo_not_click_nao_contem_anexo = processo_not_click_texto.find('Processo')
                        texto_pdf = processo_not_click_texto.find('Clique aqui')
                        if texto_pdf == 0:
                            break
                        n = 1
                        if processo_not_click_nao_possui == 0 or processo_not_click_nao_aberto == 0 or processo_not_click_nao_contem_anexo == 0:
                            driver.switch_to.default_content()
                            driver.switch_to.frame(IFRAME_ARVORE_ARQUIVO)
                            try:
                                xpath_pattern = '//*[@id[starts-with(., "ancjoin")]]'
                                if xpath_pattern:
                                    botao_mais = driver.find_elements(By.XPATH, xpath_pattern)[-2].click()
                            except Exception:
                                pass
                            driver.switch_to.default_content()
                            driver.switch_to.frame(IFRAME_CONTEUDO_DOCUMENTO)
                    ...
                    try:
                        IFRAME_ARVORE_ARQUIVO_DOC = driver.find_element(By.CSS_SELECTOR, '#divArvoreConteudo > #divArvoreHtml > iframe')
                        if IFRAME_ARVORE_ARQUIVO_DOC:
                            ...
                            inicio = assinatura_text.find('Documento assinado eletronicamente por')
                ...
                try:
                    if data_doc == '':
                        driver.switch_to.default_content()
                        driver.switch_to.frame(IFRAME_CONTEUDO_DOCUMENTO)
                        IFRAME_ARVORE_ARQUIVO_PDF = driver.find_element(By.CSS_SELECTOR, '#divArvoreConteudo > #divArvoreHtml > #divArvoreInformacao')
                        if IFRAME_ARVORE_ARQUIVO_PDF:
                            botao_click = driver.find_element(By.XPATH, '//*[@id="divArvoreAcoes"]/a[1]')
                            botao_click.click()
                            time.sleep(1)
                            data_doc = driver.find_element(By.XPATH, '//*[@id="tblAnexos"]/tbody/tr/td[3]/div').text
                            data_doc = data_doc[:10]
                except Exception as e:
                    # print(e)
                    break
                ...
                lista_id_obra.append(obra)
                lista_data_extracao.append(DATA_ATUAL)
                print(f'{len(lista_ultimo_doc)} | {len(lista_data_ult_doc)} | {len(lista_id_obra)} \n {nome_documento} | {data_doc} | {obra}')
            
            if len(lista_data_extracao) == len(lista_data_ult_doc) == len(lista_id_obra) == len(lista_ultimo_doc):
                lista_teste = True
                driver.quit()
...
df_tabela_mae_ultimo_doc_fase = pd.merge(df_tabela_mae_fase, df_tabela_mae_ultimo_doc, how='right', on=['Id Obra'])
df_tabela_mae_ultimo_doc_fase

df_tabela_mae_ultimo_doc_fase.to_excel(f'../../../../workspace/00-database/processo_mae/lista_completa/lista_{DATA_ATUAL}.xlsx', index=False)
#%%
...
lista_completa_dia_anterior = pd.read_excel(f'../../../../workspace/00-database/processo_mae/lista_completa/lista_{dia_anterior}.xlsx')
lista_completa_dia_anterior

#%%
#COMPARANDO AS LISTAS DO DIA ANTERIOR COM A ATUAL
comparativo = pd.merge(lista_completa_dia_anterior, df_tabela_mae_ultimo_doc_fase, how='right', on=['ultimo_documento'], suffixes=['_anterior',None], indicator=True).dropna(inplace=False, axis=1, how='all')
comparativo

colunas_para_manter = [col for col in comparativo.columns if not col.endswith('anterior')]
comparativo = comparativo[colunas_para_manter]
comparativo

verificacao_novos_arquivos = comparativo[comparativo['_merge']!= 'both'].drop(columns=['_merge'])
